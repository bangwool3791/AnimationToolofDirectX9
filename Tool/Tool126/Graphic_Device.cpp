...
CGraphic_Device::CGraphic_Device()
	: m_p3D(nullptr) , m_pGraphicDev(nullptr), m_pSprite(nullptr), m_pFont(nullptr)
{
}
...
HRESULT	CGraphic_Device::InitDevice(const GRAPHIC_DEVICE_DESC& _stGpDev, LPDIRECT3DDEVICE9* _ppDevice)
{
	m_stGpDev = _stGpDev;
	// HRESULT : 함수가 호출 이후 문제가 없이 종료될 경우 양수값을
	// 문제가 있는 상태로 종료될 경우 음수값을 반환시키기 위한 자료형

	// (그리기)장치 초기화 : 장치를 조사할 객체 생성(m_p3D) -> 장치 조사 -> 장치 제어 객체 생성(m_pGraphicDev)
	...
	if (FAILED(m_p3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_stGpDev.hWnd, vp, &d3dpp, &m_pGraphicDev)))
	{
	...
	*_ppDevice = m_pGraphicDev;
	return S_OK;
}

void CGraphic_Device::SetParameters(D3DPRESENT_PARAMETERS & d3dpp)
{
	d3dpp.BackBufferWidth = m_stGpDev.iWinCX;
	d3dpp.BackBufferHeight = m_stGpDev.iWinCY;
	...
	d3dpp.hDeviceWindow = m_stGpDev.hWnd;	// 장치를 사용할 윈도우 핸들 설정
...
void CGraphic_Device::Render_End(HWND hWnd)
{
	...
	m_pGraphicDev->Present(nullptr, nullptr, m_stGpDev.hWnd, nullptr);
