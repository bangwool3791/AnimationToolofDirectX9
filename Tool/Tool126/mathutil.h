...
namespace Engine
{
	...
	struct Math
	{
		...
		FORCEINLINE static int TruncToInt(float InFloat)
		{
			return (int)InFloat;
		}

		FORCEINLINE static int RountToInt(float InFloat)
		{
			return TruncToInt(roundf(InFloat));
		}

		FORCEINLINE static int FloorToInt(float InFloat)
		{
		...
		static FORCEINLINE void GetSinCosRad(float& OutSin, float& OutCos, float InRadian)
		{
